...
async function fillForm(nodeid) {
  ...
  const form = await get_file({
    url: "templates/headerForm.hbs",
    isJson: false,
  });
  const form_group = await get_file({
    url: "templates/form_group.hbs",
    isJson: false,
  });
  const form_properties = await get_file({
    url: "templates/form_properties.hbs",
    isJson: false,
  });
